                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade" id="pix-panel" role="tabpanel" aria-labelledby="pix-tab">
                        ...
                        <form id="pix-payment-form" class="mb-4">
                            ...
                            <div class="mb-3">
                                <label class="form-label" for="pix-doc-number">CPF/CNPJ</label>
                                <input type="text" class="form-control" id="pix-doc-number" placeholder="CPF ou CNPJ" maxlength="18" required>
                                <small class="form-text text-muted">Digite o CPF (11 dígitos) ou CNPJ (14 dígitos)</small>
                            </div>
    ...
    iframe[id*="form-checkout"] {
        background-color: transparent !important;
    }

    /* Forçar cor do texto nos iframes do Mercado Pago */
    ...
    #form-checkout__installments iframe,
    #form-checkout__identificationNumber iframe,
    #form-checkout__issuer iframe {
        background-color: transparent !important;
    }

    /* Aplicar estilos aos containers dos campos do Mercado Pago */
    ...
    /* Hover e focus nos campos do Mercado Pago */
    #form-checkout__cardNumber:hover,
    #form-checkout__expirationDate:hover,
    #form-checkout__securityCode:hover,
    ...
    /* CSS específico para tema escuro */
    ...
    /* Forçar tema escuro se o body tiver classe dark */
    body.dark #form-checkout__cardNumber,
    body.dark #form-checkout__expirationDate,
    body.dark #form-checkout__securityCode,
    [data-bs-theme="dark"] #form-checkout__cardNumber,
    [data-bs-theme="dark"] #form-checkout__expirationDate,
    [data-bs-theme="dark"] #form-checkout__securityCode {
        background-color: #2d3748 !important;
        color: #ffffff !important;
        border-color: #4a5568 !important;
    }

    [data-bs-theme="dark"] #form-checkout__cardNumber iframe,
    [data-bs-theme="dark"] #form-checkout__expirationDate iframe,
    [data-bs-theme="dark"] #form-checkout__securityCode iframe,
    body.dark #form-checkout__cardNumber iframe,
    body.dark #form-checkout__expirationDate iframe,
    body.dark #form-checkout__securityCode iframe {
        filter: invert(1) hue-rotate(180deg) !important;
    }
    </style>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Máscara para CPF/CNPJ do PIX (detecção automática)
                ...
                callbacks: {
                    onFormMounted: error => {
                        if (error) {
                            ...
                            // Aplicar estilos aos iframes do Mercado Pago para tema escuro
                            setTimeout(() => {
                                applyDarkThemeToMercadoPagoFields();
                            }, 1000);
                        }
                    },
                    onSubmit: event => {
            ...
            const checkInterval = setInterval(() => {
                ...
                .then(data => {
                    ...
                    if (data.status === 'approved') {
                        ...
                        setTimeout(() => {
                            window.location.href = '/payment/success?payment_id=' + paymentId + '&payment_type=pix&order_id=' + orderId;
                        }, 2000);
        ...
        function generatePixQRCode() {
            ...
            // Validar se tem o número correto de dígitos
            if (docDigits.length !== 11 && docDigits.length !== 14) {
                showPixError('CPF deve ter 11 dígitos ou CNPJ deve ter 14 dígitos.');
                return;
            }
            
            // Detectar tipo e validar documento
            let isValidDoc = false;
            let docType = '';
            ...
            if (!isValidDoc) {
                showPixError(`Por favor, informe um ${docType} válido.`);
                return;
            }
            ...
            .then(data => {
                if (data.success && data.pix_data) {
                    ...
                    console.log('Payment ID obtido da resposta:', paymentId);
                    console.log('Order ID para verificação:', orderId);
                    
                    if (!paymentId) {
                        console.error('Payment ID não encontrado na resposta da API');
                        showPixError('Erro: ID do pagamento não foi retornado. Entre em contato com o suporte.');
                        return;
                    }
                    ...
                    setTimeout(() => {
                        ...
                        if (!paymentId) {
                            ...
                            window.location.href = '{{ url('/payment/success') }}?payment_id=' + 
                                paymentId + '&preference_id=' + paymentId + '&payment_type=credit_card&order_id=' + orderId;
                        }
                    }, 2000);
                } else {
                    // Remover notificação de processamento
    ...
    // Função para aplicar tema escuro aos campos do Mercado Pago
    function applyDarkThemeToMercadoPagoFields() {
        try {
            // Detectar se está em tema escuro
            ...
            // Lista de campos do Mercado Pago
            const mpFields = [
                'form-checkout__cardNumber',
                'form-checkout__expirationDate', 
                'form-checkout__securityCode'
            ...
            mpFields.forEach(fieldId => {
                ...
                if (field) {
                    ...
                    // Tentar acessar iframe interno
                    const iframe = field.querySelector('iframe');
                    if (iframe) {
                        try {
                            // Aplicar filtro para inverter cores do conteúdo do iframe
                            iframe.style.filter = 'invert(1) hue-rotate(180deg)';
                            iframe.style.backgroundColor = 'transparent';
                            
                            // Tentar acessar documento do iframe (se permitido por CORS)
                            iframe.onload = function() {
                                try {
                                    ...
                                    // Iframe de domínio diferente, usar apenas filtro CSS
                                    console.log('CORS impediu acesso ao iframe, usando filtro CSS');
                                }
                            };
                        } catch (e) {
                            console.log('Erro ao aplicar estilos ao iframe:', e);
                        }
                    }
                }
            });
            
            console.log('Tema escuro aplicado aos campos do Mercado Pago');
        } catch (error) {
            console.error('Erro ao aplicar tema escuro:', error);
        }
    }
    </script>
    @endsection